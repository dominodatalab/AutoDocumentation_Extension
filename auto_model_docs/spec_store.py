...
def save_spec(dataset_id: str, original_filename: str, content: str) -> str:
    ...
    DatasetManager.write_file(dataset_id, relative_path, content.encode("utf-8"))
    return relative_path


def list_specs(snapshot_id: str) -> list[dict[str, Any]]:
    """Return metadata for all saved spec files."""
    layout = get_layout()
    try:
        files = DatasetManager.list_files(snapshot_id, layout.specs_dir)
